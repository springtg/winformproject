...
{
	public class Form_PD_WorkSheetByOP : COM.APSWinForm.Form_Top
	{
		...
		private void InitializeComponent()
		{
			...
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(Form_PD_WorkSheetByOP));
			...
			this.lbl_MainTitle.Text = "Daily WorkSheet By OP";
			// 
			// tbtn_Print
			// 
			...
			this.cmb_Area.Location = new System.Drawing.Point(816, 58);
			...
			this.cmb_LineGroup.TabIndex = 196;
			...
			this.lbl_LineGroup.TabIndex = 195;
			...
			this.lbl_CmpCd.Location = new System.Drawing.Point(482, 59);
			this.lbl_CmpCd.Name = "lbl_CmpCd";
			this.lbl_CmpCd.Size = new System.Drawing.Size(100, 21);
			this.lbl_CmpCd.TabIndex = 193;
			...
			this.cmb_CmpCd.Location = new System.Drawing.Point(583, 58);
			...
			this.lbl_OpCd.Location = new System.Drawing.Point(248, 57);
			...
			this.cmb_LineCd.Location = new System.Drawing.Point(583, 36);
			...
			this.lbl_Line.Location = new System.Drawing.Point(482, 36);
			...
			// 
			// Form_PD_WorkSheetByOP
			// 
			this.AutoScaleBaseSize = new System.Drawing.Size(7, 15);
			this.ClientSize = new System.Drawing.Size(1016, 666);
			this.Controls.Add(this.pnl_B);
			this.Name = "Form_PD_WorkSheetByOP";
			this.Text = "Daily WorkSheet By OP";
			this.Load += new System.EventHandler(this.Form_PD_WorkSheetByOP_Load);
		...
		private void Init_Form()
		{
			...
			{ 
				...
				//Title
				this.Text = "Daily WorkSheet By OP";
				lbl_MainTitle.Text = "Daily WorkSheet By OP"; 
 

				fgrid_WorkSheet.Set_Grid("SPD_DAILY_WORKSHEET_BSC", "1", 1, ClassLib.ComVar.This_Lang, COM.ComVar.Grid_Type.ForSearch, false);
		...
		private void Display_WorkSheet()
		{
			...
			DataTable dt_ret = Select_SPD_DAILY_WORKSHEET(factory, line_group, line_cd, op_str_ymd, cmp_cd, op_cd, mat_area);
			...
			for(int i = 0; i < dt_ret.Rows.Count; i++)
			{
				...
				now_item = dt_ret.Rows[i].ItemArray[(int)ClassLib.TBSPD_DAILY_WORKSHEET_SEARCH_BSC.IxLINE_CD - 1].ToString()
					+ dt_ret.Rows[i].ItemArray[(int)ClassLib.TBSPD_DAILY_WORKSHEET_SEARCH_BSC.IxOP_CD - 1].ToString()
					+ dt_ret.Rows[i].ItemArray[(int)ClassLib.TBSPD_DAILY_WORKSHEET_SEARCH_BSC.IxCMP_CD - 1].ToString()
					+ dt_ret.Rows[i].ItemArray[(int)ClassLib.TBSPD_DAILY_WORKSHEET_SEARCH_BSC.IxLOT - 1].ToString()
					+ dt_ret.Rows[i].ItemArray[(int)ClassLib.TBSPD_DAILY_WORKSHEET_SEARCH_BSC.IxREQ_NO - 1].ToString()
					+ dt_ret.Rows[i].ItemArray[(int)ClassLib.TBSPD_DAILY_WORKSHEET_SEARCH_BSC.IxDAY_SEQ - 1].ToString(); 
				...
				if(before_item != now_item)
				{
					...
					for(int j = 1; j <= (int)ClassLib.TBSPD_DAILY_WORKSHEET_SEARCH_BSC.IxGEN; j++)
					{
					...
					{
						if(fgrid_WorkSheet[j, (int)ClassLib.TBSPD_DAILY_WORKSHEET_SEARCH_BSC.IxGEN].ToString() == dt_ret.Rows[i].ItemArray[(int)ClassLib.TBSPD_DAILY_WORKSHEET_SEARCH_BSC.IxGEN - 1].ToString())
						{
							gen_row = j;
							sel_gen = sel_gen + "/" + fgrid_WorkSheet[gen_row, (int)ClassLib.TBSPD_DAILY_WORKSHEET_SEARCH_BSC.IxGEN].ToString();
				...
				//-------------------------------------------------------------- 
				for(int j = (int)ClassLib.TBSPD_DAILY_WORKSHEET_SEARCH_BSC.IxCS_SIZE_START; j < fgrid_WorkSheet.Cols.Count; j++)
				{
					if(fgrid_WorkSheet[gen_row, j].ToString() == dt_ret.Rows[i].ItemArray[(int)ClassLib.TBSPD_DAILY_WORKSHEET_SEARCH_BSC.IxCS_SIZE - 1].ToString())
					{
						min_size_col = (min_size_col > j) ? j : min_size_col;

						size_qty = Convert.ToInt32(dt_ret.Rows[i].ItemArray[(int)ClassLib.TBSPD_DAILY_WORKSHEET_SEARCH_BSC.IxDIR_QTY - 1].ToString()); 
				...
				fgrid_WorkSheet[fgrid_WorkSheet.Rows.Count - 1, (int)ClassLib.TBSPD_DAILY_WORKSHEET_SEARCH_BSC.IxTOT_QTY] = sum_size_qty.ToString();
...
//				{
//					if(fgrid_WorkSheet[i, (int)ClassLib.TBSPD_DAILY_WORKSHEET_SEARCH_BSC.IxGEN].ToString() == token[j])
//					{
...
//			} // end for i
			...
			fgrid_WorkSheet.Cols[(int)ClassLib.TBSPD_DAILY_WORKSHEET_SEARCH_BSC.IxMODEL_NAME].AllowMerging = true;
			fgrid_WorkSheet.Cols[(int)ClassLib.TBSPD_DAILY_WORKSHEET_SEARCH_BSC.IxSTYLE_CD].AllowMerging = true;
			...
			fgrid_WorkSheet.Subtotal(AggregateEnum.Sum, 2, (int)ClassLib.TBSPD_DAILY_WORKSHEET_SEARCH_BSC.IxLINE_CD, (int)ClassLib.TBSPD_DAILY_WORKSHEET_SEARCH_BSC.IxTOT_QTY, "Line Sum.");

			for (int i = (int)ClassLib.TBSPD_DAILY_WORKSHEET_SEARCH_BSC.IxCS_SIZE_START; i < fgrid_WorkSheet.Cols.Count; i++) 
				fgrid_WorkSheet.Subtotal(AggregateEnum.Sum, 2, (int)ClassLib.TBSPD_DAILY_WORKSHEET_SEARCH_BSC.IxLINE_CD, i, "Line Sum.");


			fgrid_WorkSheet.Subtotal(AggregateEnum.Sum, 1, -1, (int)ClassLib.TBSPD_DAILY_WORKSHEET_SEARCH_BSC.IxTOT_QTY, "Total");
			...
			//--------------------------------------------------------------
			//기타 속성 
			fgrid_WorkSheet.Cols.Frozen = (int)ClassLib.TBSPD_DAILY_WORKSHEET_SEARCH_BSC.IxCS_SIZE_START;
		...
		private void Event_Tbtn_Print()
		{
			...
			COM.Com_Form.Form_Report report = new COM.Com_Form.Form_Report("DAILY PRODUCTION ORDER SHEET BY OP", sDir, para);
			report.Show();
		...
		private void Event_Click_fgrid_WorkSheet()
		{
			...
			if(fgrid_WorkSheet[sel_row, (int)ClassLib.TBSPD_DAILY_WORKSHEET_SEARCH_BSC.IxLOT] == null
				|| fgrid_WorkSheet[sel_row, (int)ClassLib.TBSPD_DAILY_WORKSHEET_SEARCH_BSC.IxLOT].ToString() == "") return;


			string sel_gen = fgrid_WorkSheet[sel_row, (int)ClassLib.TBSPD_DAILY_WORKSHEET_SEARCH_BSC.IxGEN].ToString();

			//----------------------------------------------------------------------
			//선택한 젠더 Row 표시
			int findrow = fgrid_WorkSheet.FindRow(sel_gen, 2, (int)ClassLib.TBSPD_DAILY_WORKSHEET_SEARCH_BSC.IxGEN, false, true, false);

			if(findrow == -1) return;

			fgrid_WorkSheet.GetCellRange(findrow, (int)ClassLib.TBSPD_DAILY_WORKSHEET_SEARCH_BSC.IxGEN, findrow, fgrid_WorkSheet.Cols.Count - 1).StyleNew.BackColor = ClassLib.ComVar.ClrSel_Yellow; 
			fgrid_WorkSheet.GetCellRange(findrow, (int)ClassLib.TBSPD_DAILY_WORKSHEET_SEARCH_BSC.IxGEN, findrow, fgrid_WorkSheet.Cols.Count - 1).StyleNew.ForeColor = Color.Black;
 
			if(_BeforeGenRow != -1 && _BeforeGenRow != findrow) 
				fgrid_WorkSheet.GetCellRange(_BeforeGenRow, (int)ClassLib.TBSPD_DAILY_WORKSHEET_SEARCH_BSC.IxGEN, _BeforeGenRow, fgrid_WorkSheet.Cols.Count - 1).StyleNew.Clear(); 
		...
		private void Event_SelectedValueChanged_cmb_Factory()
		{
			...
			ClassLib.ComFunction.Set_DefaultSize_Head(fgrid_WorkSheet, 
														...
														(int)ClassLib.TBSPD_DAILY_WORKSHEET_SEARCH_BSC.IxGEN,
														(int)ClassLib.TBSPD_DAILY_WORKSHEET_SEARCH_BSC.IxCS_SIZE_START);
		...
		private void Form_PD_WorkSheetByOP_Load(object sender, System.EventArgs e)
		{
			Init_Form();
		} 
		...
		/// <returns></returns>
		private DataTable Select_SPD_DAILY_WORKSHEET(string arg_factory,
			...
			string arg_mat_area)
		{
			...
			{ 
				...
				string process_name = "PKG_SPD_WORKSHEET_SEARCH_BSC.SELECT_DAILY_WORKSHEET";
				...
				MyOraDB.Parameter_Name[6] = "ARG_MAT_AREA";
				...
				MyOraDB.Parameter_Values[6] = arg_mat_area; 
