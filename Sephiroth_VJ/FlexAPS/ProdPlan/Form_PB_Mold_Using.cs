...
{
	public class Form_PB_Mold_Using : COM.APSWinForm.Form_Top
	{
		...
		{
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(Form_PB_Mold_Using));
			...
			this.fgrid_Mold.AllowSorting = C1.Win.C1FlexGrid.AllowSortingEnum.MultiColumn;
			...
			this.lbl_SubTitle1.Text = "      Search Model";
			...
			this.Name = "Form_PB_Mold_Using";
			this.Load += new System.EventHandler(this.Form_PB_Mold_Using_Load);
		...
		private void Form_PB_Mold_Using_Load(object sender, System.EventArgs e)
		{
			Init_Form();
		}
		...
		{
			if(fgrid_Mold.Rows.Count == _RowFixed) return;
			...
			for(int i=_RowFixed; i<fgrid_Mold.Rows.Count; i++)
			{
			...
			string para = "/rfn [" + Application.StartupPath + @"\" + this.Name + ".txt] /rv V_MODEL[" + cmb_model.Columns[1].Text + "]";
		...
		private void Init_Form()
		{
			...
			this.Text = "Toolings Inventory Status(using)";
			this.lbl_MainTitle.Text = "Toolings Inventory Status(using)";
...
//			}
			...
			fgrid_Mold.Set_Grid("SPB_USING_MOLD", "1", 1, ClassLib.ComVar.This_Lang, COM.ComVar.Grid_Type.ForModify, false);
			...
			_RowFixed = fgrid_Mold.Rows.Count;

			fgrid_Mold.AllowMerging = AllowMergingEnum.Free;

			fgrid_Mold.Cols[(int)ClassLib.TBSPB_MODEL_USING.IxGR_REMARKS].AllowMerging = false;
			fgrid_Mold.Cols[(int)ClassLib.TBSPB_MODEL_USING.IxGR_MOLDE_TOT].AllowMerging = false;
			fgrid_Mold.Cols[(int)ClassLib.TBSPB_MODEL_USING.IxGR_MOLD_GEN].AllowMerging = false;
			fgrid_Mold.Cols[(int)ClassLib.TBSPB_MODEL_USING.IxGR_MSIZE_YN].AllowMerging = false;
			fgrid_Mold.Cols[(int)ClassLib.TBSPB_MODEL_USING.IxGR_SPEC_CD].AllowMerging = false;
			fgrid_Mold.Cols[(int)ClassLib.TBSPB_MODEL_USING.IxGR_MOLD_CD].AllowMerging = false;
		...
		private void Search()
		{
			fgrid_Mold.Rows.Count = _RowFixed;



			DataTable dt = Select_Model_Using_Mold();
			...
			try
			{
				old_model = dt.Rows[0].ItemArray[(int)ClassLib.TBSPB_MODEL_USING.IxDB_MODEL_CD].ToString();
			}
			catch
			{
				old_model ="";
			...
			{   
				...
				new_molde = dt.Rows[i].ItemArray[(int)ClassLib.TBSPB_MODEL_USING.IxDB_MODEL_CD].ToString();
				
				if(old_model != new_molde)
				{
					fgrid_Mold.Rows.Add();
					int sct_row = fgrid_Mold.Rows.Count-1;
					fgrid_Mold[sct_row,0]= "";
					fgrid_Mold.Rows[sct_row].StyleNew.BackColor = Color.FromArgb(135, 179, 234);
					fgrid_Mold.Rows[sct_row].Height = 2;

					old_model = new_molde;

					fgrid_Mold.Rows.Add();

					int sct_row1 = fgrid_Mold.Rows.Count-1;

					for(int j=(int)ClassLib.TBSPB_MODEL_USING.IxGR_FACTORY; j<fgrid_Mold.Cols.Count; j++)
					{
						fgrid_Mold[sct_row1, j] = dt.Rows[i].ItemArray[j-1].ToString();
					}
				}
				else
				{
					...
					for(int j=(int)ClassLib.TBSPB_MODEL_USING.IxGR_FACTORY; j<fgrid_Mold.Cols.Count; j++)
					{
						fgrid_Mold[sct_row, j] = dt.Rows[i].ItemArray[j-1].ToString();
					}
				}
			}
		...
		private DataTable Select_Model_Using_Mold()
		{
			string Proc_Name = "PKG_SPB_MOLD.SELECT_MODEL_USING_MOLD";
			...
			oraDB.Parameter_Values[1] = model_cd;
		...
		private void Save_Model_Remarks(string arg_division, string arg_factory, 
			string arg_model_cd, string arg_model_gen, string arg_mold_cd, string arg_remakrs )
		{
			string Proc_Name = "PKG_SPB_MOLD.SAVE_SPB_MODEL_MOLD_REMARK";
			...
			oraDB.Parameter_Name[3] = "ARG_MODEL_GEN";
			oraDB.Parameter_Name[4] = "ARG_MOLD_CD";
			oraDB.Parameter_Name[5] = "ARG_REMARKS";
			...
			oraDB.Parameter_Values[2] = arg_model_cd;
			oraDB.Parameter_Values[3] = arg_model_gen;
			oraDB.Parameter_Values[4] = arg_mold_cd;
			oraDB.Parameter_Values[5] = arg_remakrs;
		...
		private void tbtn_Save_Click(object sender, C1.Win.C1Command.ClickEventArgs e)
		{
			...
			for(int i=_RowFixed; i<fgrid_Mold.Rows.Count; i++)
			{
				if(fgrid_Mold[i, (int)ClassLib.TBSPB_MODEL_USING.IxGR_DIVISION] != null)
				{
					if(fgrid_Mold[i, (int)ClassLib.TBSPB_MODEL_USING.IxGR_DIVISION].ToString() != "")
					{
						string division = fgrid_Mold[i, (int)ClassLib.TBSPB_MODEL_USING.IxGR_DIVISION].ToString();
						string factory = fgrid_Mold[i, (int)ClassLib.TBSPB_MODEL_USING.IxGR_FACTORY].ToString();
						string model_cd = fgrid_Mold[i, (int)ClassLib.TBSPB_MODEL_USING.IxGR_MODEL_CD].ToString();
						string model_gen = fgrid_Mold[i, (int)ClassLib.TBSPB_MODEL_USING.IxGR_MODEL_GEN].ToString();
						string mold_cd = fgrid_Mold[i, (int)ClassLib.TBSPB_MODEL_USING.IxGR_MOLD_CD].ToString();
						string remarks   = fgrid_Mold[i, (int)ClassLib.TBSPB_MODEL_USING.IxGR_REMARKS].ToString();

						Save_Model_Remarks(division, factory, model_cd, model_gen, mold_cd, remarks);
					}
				}
			}
		...
		private void fgrid_Mold_AfterEdit(object sender, C1.Win.C1FlexGrid.RowColEventArgs e)
		{
			...
			fgrid_Mold[sct_row, (int)ClassLib.TBSPB_MODEL_USING.IxGR_DIVISION] = "U";
