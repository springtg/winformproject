...
namespace FlexBase.Yield
{
	public class Pop_Item_List : COM.PCHWinForm.Pop_Normal_Light
	{
		...
		public C1.Win.C1Command.C1OutPage obarpg_Item;
		public C1.Win.C1Command.C1OutPage obarpg_Spec;
		public C1.Win.C1Command.C1OutPage obarpg_Color;  
		...
		public Pop_Yield_Modify_withSRF _Parent_Form;
		public Form_BC_Yield_withExcel _Parent_Form_WithExcel;


		public string _ItemCd = "", _ItemName = "", _Unit = "", _SizeYN = "";
		public string _SpecCd = "", _SpecName = "";
		public string _ColorCd = "", _ColorName = "";
		public string _tabPage = ""; 
		public bool _DefaultView = false;
		...
		public Pop_Item_List(Pop_Yield_Modify_withSRF arg_parent_form,
			string arg_itemcd, 
			string arg_itemname, 
			string arg_speccd, 
			string arg_specname, 
		...
		public Pop_Item_List(Form_BC_Yield_withExcel arg_parent_form,
			string arg_itemcd, 
			string arg_itemname, 
			string arg_speccd, 
			string arg_specname, 
		...
		{ 
			...
			_Parent_Form_WithExcel = arg_parent_form;
		...
		private void InitializeComponent()
		{
            ...
            this.grid_Item.MouseMove += new System.Windows.Forms.MouseEventHandler(this.grid_MouseMove);
            // 
            // grid_Item_Sheet1
            // 
            ...
            this.lbl_ItemName.Text = "Code/ Name";
            ...
            this.grid_Spec.MouseMove += new System.Windows.Forms.MouseEventHandler(this.grid_MouseMove);
            // 
            // cmenu_spec
            // 
            ...
            this.lbl_SpecName.Text = "Code/ Name";
            ...
            this.grid_Color.MouseMove += new System.Windows.Forms.MouseEventHandler(this.grid_MouseMove);
            // 
            // grid_Color_Sheet1
            // 
...
//				}
		...
		private void Return_Item_Data()
		{
			try
			{
				...
				string withExcel_select = "";

				if(! _tabPage.Equals("") )
				{
					withExcel_select = ClassLib.ComVar.Parameter_PopUp[0];
				}
				...
				if(_tabPage.Equals("") )
				{
					...
					if(! txt_Result_ItemCd.Text.Trim().Equals("") && ! txt_Result_SepcCd.Text.Trim().Equals("") )
					{
				...
				if(! _tabPage.Equals("") )
				{
					...
					string withExcel_size_new_yn = ClassLib.ComVar.Parameter_PopUp[7]; 
 
					_Parent_Form_WithExcel.Apply_Modify_Grid(withExcel_select, withExcel_size_new_yn); 

				}  // end if(! _tabPage.Equals("") )  -> 아이템/스펙/컬러 중 하나 리턴 할 경우
				else
				{

					int sel_row = _Parent_Form.fgrid_BOMTemp.Selection.r1;

					bool exist_yn = _Parent_Form.Check_Duplicate_ItemSpecColor(sel_row,
					...
					if(_Parent_Form._OnlyRawMat && exist_yn) 
					{
					...
					else
					{
						...
						// 변경 전 데이터
						string item_cd = (_Parent_Form.fgrid_BOMTemp[sel_row, (int)ClassLib.TBSBC_YIELD_VALUE_TREE.IxITEM_CD] == null) 
							? "" : _Parent_Form.fgrid_BOMTemp[sel_row, (int)ClassLib.TBSBC_YIELD_VALUE_TREE.IxITEM_CD].ToString();
 
						string spec_cd = (_Parent_Form.fgrid_BOMTemp[sel_row, (int)ClassLib.TBSBC_YIELD_VALUE_TREE.IxSPEC_CD] == null) 
							? "" : _Parent_Form.fgrid_BOMTemp[sel_row, (int)ClassLib.TBSBC_YIELD_VALUE_TREE.IxSPEC_CD].ToString();  
				
						string color_cd = (_Parent_Form.fgrid_BOMTemp[sel_row, (int)ClassLib.TBSBC_YIELD_VALUE_TREE.IxCOLOR_CD] == null) 
							? "" : _Parent_Form.fgrid_BOMTemp[sel_row, (int)ClassLib.TBSBC_YIELD_VALUE_TREE.IxCOLOR_CD].ToString();
						...
						_Parent_Form.fgrid_BOMTemp[sel_row, (int)ClassLib.TBSBC_YIELD_VALUE_TREE.IxITEM_CD] = ClassLib.ComVar.Parameter_PopUp[0];
					
						if(_Parent_Form.fgrid_BOMTemp[sel_row, (int)ClassLib.TBSBC_YIELD_VALUE_TREE.IxTEMPLATE_CD].ToString() == _Parent_Form._RawMatCd)
						{
							_Parent_Form.fgrid_BOMTemp[sel_row, (int)ClassLib.TBSBC_YIELD_VALUE_TREE.IxITEM_NAME] = ClassLib.ComVar.Parameter_PopUp[1]; // nick name
						}

						_Parent_Form.fgrid_BOMTemp[sel_row, (int)ClassLib.TBSBC_YIELD_VALUE_TREE.IxITEM_NAME1] = ClassLib.ComVar.Parameter_PopUp[1];  // item name
						_Parent_Form.fgrid_BOMTemp[sel_row, (int)ClassLib.TBSBC_YIELD_VALUE_TREE.IxSPEC_CD] = ClassLib.ComVar.Parameter_PopUp[2];
						_Parent_Form.fgrid_BOMTemp[sel_row, (int)ClassLib.TBSBC_YIELD_VALUE_TREE.IxSPEC_NAME] = ClassLib.ComVar.Parameter_PopUp[3];
						_Parent_Form.fgrid_BOMTemp[sel_row, (int)ClassLib.TBSBC_YIELD_VALUE_TREE.IxCOLOR_CD] = ClassLib.ComVar.Parameter_PopUp[4];   
						_Parent_Form.fgrid_BOMTemp[sel_row, (int)ClassLib.TBSBC_YIELD_VALUE_TREE.IxCOLOR_NAME] = ClassLib.ComVar.Parameter_PopUp[5];  
						_Parent_Form.fgrid_BOMTemp[sel_row, (int)ClassLib.TBSBC_YIELD_VALUE_TREE.IxUNIT] = ClassLib.ComVar.Parameter_PopUp[6];
						_Parent_Form.fgrid_BOMTemp[sel_row, (int)ClassLib.TBSBC_YIELD_VALUE_TREE.IxSIZE_YN] = Convert.ToBoolean(ClassLib.ComVar.Parameter_PopUp[7] );

						// 채산값 리턴 테이블 행 생성
						// modify 일때는 채산값 그대로 처리 
						string dt_template_level = _Parent_Form.fgrid_BOMTemp[sel_row, (int)ClassLib.TBSBC_YIELD_VALUE_TREE.IxTEMPLATE_LEVEL].ToString();
						string dt_item = _Parent_Form.fgrid_BOMTemp[sel_row, (int)ClassLib.TBSBC_YIELD_VALUE_TREE.IxITEM_CD].ToString();
						string dt_spec = _Parent_Form.fgrid_BOMTemp[sel_row, (int)ClassLib.TBSBC_YIELD_VALUE_TREE.IxSPEC_CD].ToString();
						string dt_color = _Parent_Form.fgrid_BOMTemp[sel_row, (int)ClassLib.TBSBC_YIELD_VALUE_TREE.IxCOLOR_CD].ToString();
  
 
						FlexBase.Yield.Pop_Yield_Modify_withSRF._DT_Return_Key old_return_key, new_return_key;
						...
						_Parent_Form.Add_Row_DT_YieldValue(old_return_key, new_return_key);  




						string condition = "DIVISION = '" + sel_row.ToString() + "'";
						DataRow[] findrow = _Parent_Form._DT_Return.Select(condition);

						_Parent_Form.Disaply_Already_YieldValue(sel_row, findrow);

					}

				}  // end if(_tabPage.Equals("") )  -> 아이템/스펙/컬러 모두 리턴 할 경우
		...
		private void btn_Return_Click(object sender, System.EventArgs e)
		{
			...
			// yield modify 화면에서 연결 되었을 경우에는 모든 항목 리턴하기 위함
			if(_Parent_Form != null)
			{
				_tabPage = "";
			}

//			else if(_Parent_Form_WithExcel == null)
//			{
//			}
...
//		private void grid_TextTipFetch(object sender, FarPoint.Win.Spread.TextTipFetchEventArgs e)
//		{ 
//
//			string mytip = "";
//
//			//mytip = grid_Item.ActiveSheet.Cells[e.Row, e.Column].Text.ToString(); 
//
//			COM.SSP src = sender as COM.SSP;  
//
//			if(src.ActiveSheet.Rows.Count <= 0) return;
//			
//			mytip = src.ActiveSheet.Cells[e.Row, e.Column].Text.ToString();
//
//			e.TipText = mytip;
// 
//			e.ShowTip = true;
...
//		}
		...
		private void _MouseMoveTooltip(object sender, System.Windows.Forms.MouseEventArgs e)
		{
 


			COM.SSP grid = sender as COM.SSP;

			string text = null;
			if (e.Button == MouseButtons.None)
			{
				// get mouse coordinates
				int row = grid.GetCellFromPixel(0, 0, e.X, e.Y).Row;
				int col = grid.GetCellFromPixel(0, 0, e.X, e.Y).Column;
				...
				{
					// get display text 
					text = grid.ActiveSheet.Cells[row, col].Text.ToString();
			...
			// set tooltip text
			if (_ttip != null && _ttip.GetToolTip(grid) != text)
				_ttip.SetToolTip(grid, text);
		...
		private void grid_MouseMove(object sender, System.Windows.Forms.MouseEventArgs e)
		{
			try
			{
				_MouseMoveTooltip(sender, e);
			}
			catch(Exception ex)
			{
				ClassLib.ComFunction.User_Message(ex.Message, "grid_MouseMove", MessageBoxButtons.OK, MessageBoxIcon.Error);
			} 
