...
namespace FlexMRP.MRP
{
	public class Form_BM_MRP_List : COM.PCHWinForm.Form_Top, IOperation
	{
        ...
        private int _outsideYnCol   = (int)TBSBM_MRP_ADJUST_MULTI.IxOUT_SIDE_YN;
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Form_BM_MRP_List));
            ...
            this.com_bom.Size = new System.Drawing.Size(208, 20);
            ...
            this.cmb_search.Size = new System.Drawing.Size(208, 20);
            ...
            this.cmb_reqReason.Size = new System.Drawing.Size(208, 20);
            ...
            this.cmb_obstype.Size = new System.Drawing.Size(208, 20);
            ...
            this.cmb_longyn.Size = new System.Drawing.Size(208, 20);
            ...
            this.cmb_outsideyn.Size = new System.Drawing.Size(208, 20);
            ...
            this.cmb_shipyn.Size = new System.Drawing.Size(208, 20);
...
//			}
		...
		{						
			...
			// form set
			this.Text = "Check MRP Result-1";
            lbl_MainTitle.Text = "Check MRP Result-1";
...
//		}
        ...
        private void Grid_AfterEditProcess()
        {
            ...
            int vSearchLevel  = Convert.ToInt16(cmb_search.SelectedValue.ToString());
            if (fgrid_main.Rows[vRow].Node.Level == vSearchLevel && vCol == _confirmQtyCol) //confirm수량 수정
            {
                foreach (int iRow in vSels)
                {
                    if (fgrid_main.Rows[iRow].Node.Level == vSearchLevel)
                    {
                        fgrid_main[iRow, vCol] = fgrid_main[vRow, vCol];
                        if (vSearchLevel == 1)   //item
                            Grid_QtyCalculation(iRow);
                        else fgrid_main.Update_Row(iRow);
                       
                    }
                }
            }
            else if (fgrid_main.Rows[vRow].Node.Level == vSearchLevel && (vCol == _shipYnCol || vCol == this._outsideYnCol))  //ship yn + outside yn수정
            {
                foreach (int iRow in vSels)
                {
                    if (fgrid_main.Rows[iRow].Node.Level == vSearchLevel)
                    {
                        int iChildCnt = fgrid_main.Rows[iRow].Node.Children;
                        for (int itRow = iRow; itRow <= iRow + iChildCnt; itRow++)
                        {
                            fgrid_main[itRow, vCol] = fgrid_main[vRow, vCol];
                            fgrid_main.Update_Row(itRow);
                        }
                    }
                }
            }
            ...
            //    foreach (int iRow in vSels)
            //    {
            //        if (fgrid_main.Rows[iRow].Node.Level == 1)
            //        {
            //            fgrid_main[iRow, vCol] = fgrid_main[vRow, vCol];
            //            Grid_QtyCalculation(iRow);
            ...
            //    }
            //}
            //else if (fgrid_main.Rows[vRow].Node.Level == 1 && (vCol == _shipYnCol || vCol == this._outsideYnCol))
            //{
            //    foreach (int iRow in vSels)
            //    {
            //        if (fgrid_main.Rows[iRow].Node.Level == 1)
...
//			}
		...
		private void Mnu_ValueChange()
		{
			try
			{
                ...
                if (cmb_search.SelectedValue.ToString().Equals("2"))  // lot별
                {
                    if (ClassLib.ComVar.Parameter_PopUp != null)
                    {
                        foreach (int vRow in fgrid_main.Selections)
                ...
                else   //item
                {
                    if (ClassLib.ComVar.Parameter_PopUp != null)
                    {
                        foreach (int iRow in fgrid_main.Selections)
                        {
                            if (fgrid_main.Rows[iRow].Node.Level == 1)
                            {
                                fgrid_main[iRow, vCol] = ClassLib.ComVar.Parameter_PopUp[0];
                                int iChildCnt = fgrid_main.Rows[iRow].Node.Children;
                                for (int itRow = iRow; itRow <= iRow + iChildCnt; itRow++)
                                {
                                    fgrid_main[itRow, vCol] = ClassLib.ComVar.Parameter_PopUp[0];
                                    fgrid_main.Update_Row(itRow);
                                }
                            }
                        }
		...
		private void Grid_QtyCalculation(int arg_row)
		{
			try
			{
				...
				for (int vRow = vStartRow ; vRow < vEndRow ; vRow++)
				{
					...
					vTempQty = (int)Math.Ceiling((vCurQty / vOldQty) * vNewQty);
			...
			try
			{
				...
				switch (arg_type)
				{
					...
					case _mnu_value:
                        ...
                        if (fgrid_main.Col != this._shipYnCol && fgrid_main.Col != this._outsideYnCol)
						{
							return false;
						}
						break;
