...
Begin VB.Form frmMO01VJ 
   ...
   Begin Threed.SSFrame ssfMenu 
      ...
      Begin Threed.SSCommand SSCommand3 
         ...
         EndProperty
         Font3D          =   3
         MouseIcon       =   "frmMO01VJ.frx":08CA
         Picture         =   "frmMO01VJ.frx":08E6
      End
      ...
      Begin Threed.SSCommand cmdExam 
         ...
         EndProperty
      End
      Begin Threed.SSCommand cmdChkScn 
         Height          =   495
         Left            =   60
         TabIndex        =   58
         Top             =   2805
   ...
   Begin VSFlex7LCtl.VSFlexGrid fspINFO 
      ...
      FormatString    =   $"frmMO01VJ.frx":0902
      ScrollTrack     =   0   'False
      ScrollBars      =   2
      ScrollTips      =   0   'False
      MergeCells      =   0
   ...
   Begin Threed.SSFrame ssfLine 
      ...
      Begin VSFlex7LCtl.VSFlexGrid fspLine 
         ...
         ColWidthMax     =   0
         ExtendLastCol   =   0   'False
         FormatString    =   $"frmMO01VJ.frx":095A
         ScrollTrack     =   0   'False
         ScrollBars      =   2
         ScrollTips      =   0   'False
         MergeCells      =   0
...
Private Function fnRstUP(arg_AREA As String, arg_LINE As String) As Long
   Dim i As Long
   Dim TOT As Long
   ...
   If arg_LINE = "ALL" Then
      TOT = 0
      For i = 0 To UBound(varRST_UP, 2) Step 1
         If CStr(varRST_UP(0, i)) = arg_AREA Then
            TOT = TOT + CLng(varRST_UP(2, i))
'            If i < UBound(varCOMP, 2) Then
'               If varCOMP(0, i) <> varCOMP(0, i + 1) Then
'                  Exit For
'               End If
'            End If
   ...
   Else
      For i = 0 To UBound(varRST_UP, 2) Step 1
         If CStr(varRST_UP(0, i)) = arg_AREA And CStr(varRST_UP(1, i)) = arg_LINE Then
            fnRstUP = CLng(varRST_UP(2, i))
            Exit Function
         End If
      Next i
      fnRstUP = 0
...
Private Sub sbDspData3(ByVal arg_pos As Integer)
   ...
   'Displaying WIP
   'Upper
   lbWIP(0).Caption = fnOldWIP("UPS1.PO.UP", gLine) + CLng(lbrst(1).Caption) - fnRstUP("1", gLine)
   lbWIP(1).Caption = fnOldWIP("UPS2.IP.UP", gLine) + CLng(lbrst(0).Caption) - CLng(lbrst(2).Caption)
   lbWIP(20).Caption = fnOldWIP("UPS2.PO.UP", gLine) + CLng(lbrst(2).Caption) - fnRstUP("2", gLine)
...
Private Sub Form_Load()
   ...
   SQL = "SELECT nvl(MAX(TO_NUMBER(PLANT_CD)),10) FROM NA_GEN_PARA "
   varV = fnGetOraData(SQL)
   If IsArray(varV) Then
      gASSY_LINE_NUM = CInt(varV(0, 0))
   Else
      If varV = "" Then
         MsgBox "NA_GEN_PARA Data is nothing"
      Else
         MsgBox varV
      End If
   ...
   fspINFO.Rows = gASSY_LINE_NUM
   For i = 0 To gASSY_LINE_NUM - 1
      fspINFO.TextMatrix(i, 0) = i + 1
   Next i
...
Private Sub sbBarScan()
   ...
   If Not IsArray(varSCN) Then
      ...
      'chkAutoScan.Value = 0
      If varSCN = "" Then
         Call sbMsgDsp("Can not find Applyed Data!", gMsgDspSec)
      Else
...
Public Sub sbDspData(ByVal arg_ymd As String, ByVal arg_LINE As String)
   ...
   SQL = ""
   SQL = SQL & " SELECT SUBSTR(UPD_USER,5,1), ASSY_LINE, SUM(PRS_QTY) "
   SQL = SQL & "   FROM MP_PROD "
   
   If ssoMD(0).Value Then
      SQL = SQL & "  WHERE RST_YMD = '" & arg_ymd & "' "
   Else
      SQL = SQL & "  WHERE RST_YMD >= '" & Mid(arg_ymd, 1, 6) & "01' AND RST_YMD <= '" & arg_ymd & "' "
   End If
   SQL = SQL & "    AND PROC     = 'FGA'"
   SQL = SQL & "    AND RST_DIV  = 'I'"
   SQL = SQL & "    AND SEMI_GOOD_CD = 'UP'"
   
   SQL = SQL & "  GROUP BY SUBSTR(UPD_USER,5,1), ASSY_LINE "
   
   Screen.MousePointer = 11
   Set varRST_UP = Nothing
   varRST_UP = fnGetOraData(SQL)
   Screen.MousePointer = 1
   If Not IsArray(varRST_UP) Then
      If varRST_UP = "" Then
         Call sbMsgDsp("Can not find Components Production Results Data!", gMsgDspSec)
      Else
         Call sbMsgDsp(varRST_UP, gMsgDspSec)
      End If
   End If
...
Private Sub sbDspData2(ByVal arg_LINE As String)
   ...
   lbWIP(0).Caption = fnOldWIP("UPS1.PO.UP", gLine) + CLng(lbrst(1).Caption) - fnRstUP("1", gLine)
   lbWIP(1).Caption = fnOldWIP("UPS2.IP.UP", gLine) + CLng(lbrst(0).Caption) - CLng(lbrst(2).Caption)
   lbWIP(20).Caption = fnOldWIP("UPS2.IP.UP", gLine) + CLng(lbrst(2).Caption) - fnRstUP("2", gLine)
...
Private Sub sbLine_SUB(arg_OPCD As String, arg_RstDiv As String, arg_COMP As String, arg_HeadName As String, arg_Col As Long)
   ...
   If i >= 0 Then
      ...
      For j = varPOS(i) To UBound(varRST, 2) Step 1
         ...
         If strV1 = strV2 Then
            fg = True
            xRow = IIf(CInt(varRST(3, j)) = 99, 17, CInt(varRST(3, j)))
            fspLine.TextMatrix(xRow, arg_Col) = fnNVZ(varRST(4, j))
         End If
      Next j
   End If
End Sub
   ...
   If i >= 0 Then
      ...
      For j = varPOS(i) To UBound(varRST, 2) Step 1
         ...
         If strV1 = strV2 Then
            fg = True
            xRow = IIf(CInt(varRST(3, j)) = 99, 17, CInt(varRST(3, j)))
            fspLine.TextMatrix(xRow, fspLine.Cols - 3) = CStr(CLng(fnNVZ(fspLine.TextMatrix(xRow, fspLine.Cols - 3))) + CLng(fnNVZ(varRST(5, j))))
            fspLine.TextMatrix(xRow, fspLine.Cols - 2) = CStr(CLng(fnNVZ(fspLine.TextMatrix(xRow, fspLine.Cols - 2))) + CLng(fnNVZ(varRST(6, j))))
            fspLine.TextMatrix(xRow, fspLine.Cols - 1) = CStr(CLng(fnNVZ(fspLine.TextMatrix(xRow, fspLine.Cols - 1))) + CLng(fnNVZ(varRST(7, j))))
         End If
      Next j
   End If
End Sub
  ...
  'Setting Plan Data
   ...
   For i = 0 To UBound(varPLN, 2) Step 1
      ...
      If varPLN(0, i) = strDept1 Then
         fg = True
         xRow = IIf(CInt(varPLN(1, i)) = 99, 17, CInt(varPLN(1, i)))
         fspLine.TextMatrix(xRow, 1) = fnNVZ(varPLN(2, i))
      End If
   Next i
   ...
   If strDept2 <> "*" Then
      ...
      For i = 0 To UBound(varPLN, 2) Step 1
         ...
         If varPLN(0, i) = strDept2 Then
            fg = True
            xRow = IIf(CInt(varPLN(1, i)) = 99, 17, CInt(varPLN(1, i)))
            fspLine.TextMatrix(xRow, 1) = CStr(CLng(IIf(fspLine.TextMatrix(xRow, 1) = "", "0", fspLine.TextMatrix(xRow, 1))) + CLng(fnNVZ(varPLN(2, i))))
         End If
      Next i
   End If
         ...
         For i = 0 To UBound(varWIP, 2) Step 1
            If vLnPOS(II, JJ) = varWIP(0, i) Then
               ...
               If CStr(varWIP(0, i)) = "UPS1.PO.UP" Then
                  fspLine.TextMatrix(CInt(varWIP(1, i)), JJ + 2) = CStr(CLng(varWIP(2, i)) + fnLineRST(InKEY, CStr(varWIP(1, i))) - fnRstUP("1", CStr(varWIP(1, i))))
                  'fnLineRST(OutKEY, CStr(varWIP(1, i))) + fnLineRST("UPS2.P.UP", CStr(varWIP(1, i))))
                  
               ElseIf CStr(varWIP(0, i)) = "UPS2.PO.UP" Then
                  fspLine.TextMatrix(CInt(varWIP(1, i)), JJ + 2) = CStr(CLng(varWIP(2, i)) + fnLineRST(InKEY, CStr(varWIP(1, i))) - fnRstUP("2", CStr(varWIP(1, i))))
...
Private Sub sbCtlDept2(ByRef obj As Object, ByRef obj2 As Object)
Dim i As Integer
...
Private Sub sbCtlDeptAllOut()
    ...
    If sspOS2(0).BevelOuter = 1 Then
       Call sbCtlDept2(sspOS2, lblBtm2(0))
    End If
    If sspPU2(0).BevelOuter = 1 Then
       Call sbCtlDept2(sspPU2, lblBtm2(1))
    End If
    If sspPH2(0).BevelOuter = 1 Then
       Call sbCtlDept2(sspPH2, lblBtm2(2))
    End If
...
Private Sub lbrst_MouseMove(Index As Integer, Button As Integer, Shift As Integer, X As Single, Y As Single)
   ...
   lbfsspu.Visible = False
   lbfsssp.Visible = False
   lbfssph.Visible = False
   lblUP1.Visible = False
   lblUP2.Visible = False
...
Private Sub lbWIP_MouseMove(Index As Integer, Button As Integer, Shift As Integer, X As Single, Y As Single)
   Dim ssfTOP As Long
   Dim ssfLEFT As Long
   ...
   If Not vFlag Then
      ...
      Case 0
         lbw.Caption = CStr(fnOldWIP("UPS1.PO.UP", gLine))
         ssfTOP = ssfUP.Top + sspUP(3).Top + 50
         ssfLEFT = ssfUP.Left + sspUP(3).Left
         lblUP1.Caption = fnRstUP("1", gLine)
         lblUP1.Visible = True
      Case 1
         lbw.Caption = CStr(fnOldWIP("UPS2.IP.UP", gLine))
         ssfTOP = ssfUP.Top + sspUP(0).Top + 50
         ssfLEFT = ssfUP.Left + sspUP(0).Left
      Case 2
         lbw.Caption = CStr(fnOldWIP("OSP.TP.OS", gLine))
         ssfTOP = ssfOS.Top + sspOS(1).Top + 50
         ssfLEFT = ssfOS.Left + sspOS(1).Left
      Case 3
         lbw.Caption = CStr(fnOldWIP("OSP.PO.OS", gLine))
         ssfTOP = ssfOS.Top + sspOS(2).Top + 50
         ssfLEFT = ssfOS.Left + sspOS(2).Left
      Case 4
         lbw.Caption = CStr(fnOldWIP("PUS.TP.PU", gLine))
         ssfTOP = ssfPU.Top + sspPU(1).Top + 50
         ssfLEFT = ssfPU.Left + sspPU(1).Left
      Case 5
         lbw.Caption = CStr(fnOldWIP("PUS.PO.PU", gLine))
         ssfTOP = ssfPU.Top + sspPU(4).Top + 50
         ssfLEFT = ssfPU.Left + sspPU(4).Left
      Case 6
         lbw.Caption = CStr(fnOldWIP("SPP.TP.SP", gLine))
         ssfTOP = ssfPU.Top + sspPU(3).Top + 50
         ssfLEFT = ssfPU.Left + sspPU(3).Left
      Case 7
         lbw.Caption = CStr(fnOldWIP("SPP.PO.SP", gLine))
         ssfTOP = ssfPU.Top + sspPU(5).Top + 50
         ssfLEFT = ssfPU.Left + sspPU(5).Left
      Case 8
         lbw.Caption = CStr(fnOldWIP("PHP.IO.PH", gLine))
         ssfTOP = ssfPH.Top + sspPH(0).Top + 50
         ssfLEFT = ssfPH.Left + sspPH(0).Left
      Case 9
         lbw.Caption = CStr(fnOldWIP("PHI.IO.PH", gLine))
         ssfTOP = ssfPH.Top + sspPH(1).Top + 50
         ssfLEFT = ssfPH.Left + sspPH(1).Left
      Case 10
         lbw.Caption = CStr(fnOldWIP("FSS.IP.OS", gLine))
         ssfTOP = ssfFSS.Top + sspFSS(0).Top + 50
         ssfLEFT = ssfFSS.Left + sspFSS(0).Left
      Case 11
         lbw.Caption = CStr(fnOldWIP("FSS.IP.PU", gLine))
         lbfsspu.Caption = CStr(fnFssCOMP("PU", gLine))
         lbfsspu.Visible = True
         ssfTOP = ssfFSS.Top + sspFSS(1).Top + 50
         ssfLEFT = ssfFSS.Left + sspFSS(1).Left
      Case 12
         lbw.Caption = CStr(fnOldWIP("FSS.IP.SP", gLine))
         lbfsssp.Caption = CStr(fnFssCOMP("SP", gLine))
         lbfsssp.Visible = True
         ssfTOP = ssfFSS.Top + sspFSS(2).Top + 50
         ssfLEFT = ssfFSS.Left + sspFSS(2).Left
      Case 13
         lbw.Caption = CStr(fnOldWIP("FSS.IP.PH", gLine))
         lbfssph.Caption = CStr(fnFssCOMP("PH", gLine))
         lbfssph.Visible = True
         ssfTOP = ssfFSS.Top + sspFSS(3).Top + 50
         ssfLEFT = ssfFSS.Left + sspFSS(3).Left
      Case 14
         lbw.Caption = CStr(fnOldWIP("FSS.PO.FS", gLine))
         ssfTOP = ssfFSS.Top + sspFSS(4).Top + 50
         ssfLEFT = ssfFSS.Left + sspFSS(4).Left
      Case 15
         lbw.Caption = CStr(fnOldWIP("FGA.IT.UP", gLine))
         ssfTOP = ssfFGA.Top + sspFGA(0).Top + 50
         ssfLEFT = ssfFGA.Left + sspFGA(0).Left
      Case 16
         lbw.Caption = CStr(fnOldWIP("FGA.TP.UP", gLine))
         ssfTOP = ssfFGA.Top + sspFGA(1).Top + 50
         ssfLEFT = ssfFGA.Left + sspFGA(1).Left
      Case 17
         lbw.Caption = CStr(fnOldWIP("FGA.IT.FS", gLine))
         ssfTOP = ssfFGA.Top + sspFGA(2).Top + 50
         ssfLEFT = ssfFGA.Left + sspFGA(2).Left
      
      Case 18
         lbw.Caption = CStr(fnOldWIP("FGA.TP.FS", gLine))
         ssfTOP = ssfFGA.Top + sspFGA(3).Top + 50
         ssfLEFT = ssfFGA.Left + sspFGA(3).Left
      Case 19
         lbw.Caption = CStr(fnOldWIP("FGA.PO.FG", gLine))
         ssfTOP = ssfFGA.Top + sspFGA(4).Top + 50
         ssfLEFT = ssfFGA.Left + sspFGA(4).Left
      Case 20
         lbw.Caption = CStr(fnOldWIP("UPS2.PO.UP", gLine))
         ssfTOP = ssfUP.Top + sspUP(3).Top + 50
         ssfLEFT = ssfUP.Left + sspUP(3).Left
         lblUP2.Caption = fnRstUP("2", gLine)
         lblUP2.Visible = True
      End Select
