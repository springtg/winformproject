...
{ 
	public class Pop_PB_Rout : COM.APSWinForm.Pop_Large
	{
		...
		private void InitializeComponent()
		{
			...
			this.addflow_StdRout.CanDrawLink = false;
			...
			this.menuItem_NodeProp.Visible = false;
			...
			this.menuItem_LinkProp.Visible = false;
...
// 				for(int i = _Rowfixed; i < fgrid_Rout.Rows.Count; i++)
		...
		{ 
			try
			{
				...
				//Next Routing Sequence 자동 할당
				string next_seq = Get_NextSeq();

				//factory, rout_seq, op_cd
				//ClassLib.ComVar.Parameter_PopUp = new string[] {cmb_Factory.SelectedValue.ToString(), "", ""};
				ClassLib.ComVar.Parameter_PopUp = new string[] {cmb_Factory.SelectedValue.ToString(), next_seq, ""};
				...
				//---------------------------------------------------------------
				//Add Node
				Lassalle.Flow.Node before_node = new Lassalle.Flow.Node(); 
				...
				{
					before_node = addflow_StdRout.Nodes[ (_Node_Index - 1) ];
 
					_AddNode = addflow_StdRout.Nodes.Add(before_node.Location.X, before_node.Location.Y + (before_node.Size.Height * 4), 
						before_node.Size.Width, before_node.Size.Height);
				...
				fgrid_Rout[fgrid_Rout.Rows.Count - 1, (int)ClassLib.TBSPB_ROUT.IxOP_NAME] = ClassLib.ComVar.Parameter_PopUp[3];
		...
		{
			// 첫번째 Row, 라우팅 공정 하나면 링크 그리지 않음
			if(fgrid_Rout.Rows.Count <= _Rowfixed + 1) return; 
 
			Lassalle.Flow.Link link = new Lassalle.Flow.Link();
  
			link = addflow_StdRout.Nodes[_AddNode.Index].InLinks.Add(addflow_StdRout.Nodes[ (_AddNode.Index - 1) ]); 
			...
			for(int i = fgrid_LinkDef.Rows.Fixed; i < fgrid_LinkDef.Rows.Count; i++)
			{
				if(fgrid_LinkDef[i, (int)ClassLib.TBSPB_LINK_DEF.IxLINK_TYPE].ToString() == ClassLib.ComVar.RoutLinkType)
				{ 
					ClassLib.ComFunction.Set_LinkProp(fgrid_LinkDef, link, i);
		...
		/// <summary>
		/// Get_NextSeq : Next Routing Sequence 자동 할당
		/// </summary>
		/// <returns></returns>
		private string Get_NextSeq()
		{
			int maxseq = 0, nextseq = 0;

			if(fgrid_Rout.Rows.Count > fgrid_Rout.Rows.Fixed)
			{
				maxseq = Convert.ToInt32(fgrid_Rout[fgrid_Rout.Rows.Count - 1, (int)ClassLib.TBSPB_ROUT.IxROUT_SEQ].ToString() );
			}

			nextseq = maxseq + 10;

			return Convert.ToString(nextseq);
		}
			...
			try
			{
				...
				for(int i = _Rowfixed; i < fgrid_Rout.Rows.Count; i++)
				{
					...
					if((node.Tag).ToString() == fgrid_Rout[i, (int)ClassLib.TBSPB_ROUT.IxH_ROUT_SEQ].ToString())
					{
						...
						fgrid_Rout[i, (int)ClassLib.TBSPB_ROUT.IxOP_NAME] = ClassLib.ComVar.Parameter_PopUp[3];
					}
				}
