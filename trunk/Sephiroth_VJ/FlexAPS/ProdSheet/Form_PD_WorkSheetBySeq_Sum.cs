...
{
	public class Form_PD_WorkSheetBySeq_Sum : COM.APSWinForm.Form_Top
	{
		...
		public Form_PD_WorkSheetBySeq_Sum()
		{
		...
		private void InitializeComponent()
		{
			...
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(Form_PD_WorkSheetBySeq_Sum));
			...
			// 
			// Form_PD_WorkSheetBySeq_Sum
			// 
			this.AutoScaleBaseSize = new System.Drawing.Size(7, 15);
			this.ClientSize = new System.Drawing.Size(1016, 666);
			this.Controls.Add(this.pnl_B);
			this.Name = "Form_PD_WorkSheetBySeq_Sum";
			this.Text = "Daily WorkSheet By Sequence ";
			this.Load += new System.EventHandler(this.Form_PD_WorkSheetBySeq_Sum_Load);
		...
		private void Init_Form()
		{
			...
			{ 
				...
				//Title
				this.Text = "Daily WorkSheet By Sequence - VSM";
				lbl_MainTitle.Text = "Daily WorkSheet By Sequence - VSM";
 

				fgrid_WorkSheet.Set_Grid("SPD_DAILY_WORKSHEET_TS_BSC", "2", 1, ClassLib.ComVar.This_Lang, COM.ComVar.Grid_Type.ForSearch, false);
		...
		private void Display_WorkSheet()
		{
			...
			DataTable dt_ret = Select_SPD_DAILY_WORKSHEET_TS_SUM(factory, line_group, line_cd, op_str_ymd, cmp_cd, op_cd);
			...
			for(int i = 0; i < dt_ret.Rows.Count; i++)
			{
				...
				now_item = dt_ret.Rows[i].ItemArray[(int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxLINE_CD - 1].ToString() 
					+ dt_ret.Rows[i].ItemArray[(int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxOP_CD - 1].ToString()
					+ dt_ret.Rows[i].ItemArray[(int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxCMP_CD - 1].ToString()
					+ dt_ret.Rows[i].ItemArray[(int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxLOT - 1].ToString()
					+ dt_ret.Rows[i].ItemArray[(int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxREQ_NO - 1].ToString()
					+ dt_ret.Rows[i].ItemArray[(int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxDAY_SEQ - 1].ToString()
					+ dt_ret.Rows[i].ItemArray[(int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxINPUT_PRIO - 1].ToString(); 
				...
				if(before_item != now_item)
				{
					...
					for(int j = 1; j <= (int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxGEN; j++)
					{
					...
					{
						if(fgrid_WorkSheet[j, (int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxGEN].ToString() == dt_ret.Rows[i].ItemArray[(int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxGEN - 1].ToString())
						{
							gen_row = j;
							sel_gen = sel_gen + "/" + fgrid_WorkSheet[gen_row, (int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxGEN].ToString();
				...
				//-------------------------------------------------------------- 
				for(int j = (int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxCS_SIZE_START; j < fgrid_WorkSheet.Cols.Count; j++)
				{
					if(fgrid_WorkSheet[gen_row, j].ToString() == dt_ret.Rows[i].ItemArray[(int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxCS_SIZE - 1].ToString())
					{
						min_size_col = (min_size_col > j) ? j : min_size_col;

						size_qty = Convert.ToInt32(dt_ret.Rows[i].ItemArray[(int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxDIR_QTY - 1].ToString()); 
				...
				fgrid_WorkSheet[fgrid_WorkSheet.Rows.Count - 1, (int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxTOT_QTY] = sum_size_qty.ToString();
			...
			//				{
			//					if(fgrid_WorkSheet[i, (int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxGEN].ToString() == token[j])
			//					{
			...
			fgrid_WorkSheet.Cols[(int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxMODEL_NAME].AllowMerging = true;
			fgrid_WorkSheet.Cols[(int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxSTYLE_CD].AllowMerging = true;
			...
			fgrid_WorkSheet.Subtotal(AggregateEnum.Sum, 2, (int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxREQ_NO, (int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxTOT_QTY, "Req. Sum.");

			for (int i = (int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxCS_SIZE_START; i < fgrid_WorkSheet.Cols.Count; i++) 
				fgrid_WorkSheet.Subtotal(AggregateEnum.Sum,2, (int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxREQ_NO, i, "Req. Sum.");
 
   
			fgrid_WorkSheet.Subtotal(AggregateEnum.Sum, 1, (int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxLINE_CD, (int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxTOT_QTY, "Line Sum.");

			for (int i = (int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxCS_SIZE_START; i < fgrid_WorkSheet.Cols.Count; i++) 
				fgrid_WorkSheet.Subtotal(AggregateEnum.Sum, 1, (int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxLINE_CD, i, "Line Sum.");
 

			fgrid_WorkSheet.Subtotal(AggregateEnum.Sum, 0, -1, (int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxTOT_QTY, "Total");
			...
			//--------------------------------------------------------------
			//기타 속성 
			fgrid_WorkSheet.Cols.Frozen = (int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxCS_SIZE_START;
		...
		private void Event_Tbtn_Print()
		{
			...
			COM.Com_Form.Form_Report report = new COM.Com_Form.Form_Report("DAILY PRODUCTION ORDER SHEET BY SEQUENCE (VSM)", sDir, para);
			report.Show();
		...
		private void Event_Click_fgrid_WorkSheet()
		{
			...
			if(fgrid_WorkSheet[sel_row, (int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxLOT] == null
				|| fgrid_WorkSheet[sel_row, (int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxLOT].ToString() == "") return;


			string sel_gen = fgrid_WorkSheet[sel_row, (int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxGEN].ToString();

			//----------------------------------------------------------------------
			//선택한 젠더 Row 표시
			int findrow = fgrid_WorkSheet.FindRow(sel_gen, 2, (int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxGEN, false, true, false);

			if(findrow == -1) return;

			fgrid_WorkSheet.GetCellRange(findrow, (int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxGEN, findrow, fgrid_WorkSheet.Cols.Count - 1).StyleNew.BackColor = ClassLib.ComVar.ClrSel_Yellow; 
			fgrid_WorkSheet.GetCellRange(findrow, (int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxGEN, findrow, fgrid_WorkSheet.Cols.Count - 1).StyleNew.ForeColor = Color.Black;
 
			if(_BeforeGenRow != -1 && _BeforeGenRow != findrow) 
				fgrid_WorkSheet.GetCellRange(_BeforeGenRow, (int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxGEN, _BeforeGenRow, fgrid_WorkSheet.Cols.Count - 1).StyleNew.Clear(); 
		...
		private void Event_SelectedValueChanged_cmb_Factory()
		{
			...
			ClassLib.ComFunction.Set_DefaultSize_Head(fgrid_WorkSheet, 
				...
				(int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxGEN,
				(int)ClassLib.TBSPD_DAILY_WORKSHEET_TS_SUM_SEARCH_BSC.IxCS_SIZE_START);
		...
		private void Form_PD_WorkSheetBySeq_Sum_Load(object sender, System.EventArgs e)
		{
			Init_Form();
		} 
		...
		/// <returns></returns>
		private DataTable Select_SPD_DAILY_WORKSHEET_TS_SUM(string arg_factory,
			...
			string arg_op_cd)
		{
			...
			{ 
				...
				string process_name = "PKG_SPD_WORKSHEET_SEARCH_BSC.SELECT_DAILY_WORKSHEET_TS_VSM";
