...
{
    public partial class Pop_Prod_Result_VJ : COM.PCHWinForm.Pop_Large_Light
    {
        ...
        Form_Prod_Result_OPCD_VJ _main_form_vj = null;
        public bool save_flg = false;
        #endregion

        #region Resource
        public Pop_Prod_Result_VJ()
        {
            InitializeComponent();
        }
        
        public Pop_Prod_Result_VJ(string arg_pcard_id, string arg_op_cd, string arg_rtd_div, string arg_op_name, string arg_factory, string arg_category, string arg_season, string arg_model, string arg_color, string arg_sample_type, Form_Prod_Result_OPCD_VJ arg_form)
        {
        ...
        private void Init_Form()
        {
            ...
            #region Grid Setting
            fgrid_result.Set_Grid_CDC("SXG_PROD_RESULT_POP_VJ", "3", 2, COM.ComVar.This_Lang, COM.ComVar.Grid_Type.ForSearch, false);           
            fgrid_result.Rows.Count = fgrid_result.Rows.Fixed;
            //fgrid_result.ExtendLastCol = false;
            
            if (tmp_rst_div.Equals("I"))
            {
                fgrid_result[fgrid_result.Rows.Fixed - 2, (int)ClassLib.TBSXG_PROD_RESULT_POP_NEW.IxRST_QTY] = "Qty";
                fgrid_result[fgrid_result.Rows.Fixed - 1, (int)ClassLib.TBSXG_PROD_RESULT_POP_NEW.IxRST_QTY] = "Incoming";
            }
            else if (tmp_rst_div.Equals("T"))
            {
                fgrid_result[fgrid_result.Rows.Fixed - 2, (int)ClassLib.TBSXG_PROD_RESULT_POP_NEW.IxRST_QTY] = "Qty";
                fgrid_result[fgrid_result.Rows.Fixed - 1, (int)ClassLib.TBSXG_PROD_RESULT_POP_NEW.IxRST_QTY] = "Input";
            }
            else if (tmp_rst_div.Equals("P"))
            {
                fgrid_result[fgrid_result.Rows.Fixed - 2, (int)ClassLib.TBSXG_PROD_RESULT_POP_NEW.IxRST_QTY] = "Qty";
                fgrid_result[fgrid_result.Rows.Fixed - 1, (int)ClassLib.TBSXG_PROD_RESULT_POP_NEW.IxRST_QTY] = "Product";
            }
            else if (tmp_rst_div.Equals("O"))
            {
                fgrid_result[fgrid_result.Rows.Fixed - 2, (int)ClassLib.TBSXG_PROD_RESULT_POP_NEW.IxRST_QTY] = "Qty";
                fgrid_result[fgrid_result.Rows.Fixed - 1, (int)ClassLib.TBSXG_PROD_RESULT_POP_NEW.IxRST_QTY] = "Outgoing";
            }
        ...
        private void btn_confirm_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    int sct_row = _main_form_vj.fgrid_result.Selection.r1;
                    int start_col = (int)ClassLib.TBSXG_PROD_RESULT_OP.IxUPC_DIR;
                    int end_col = (int)ClassLib.TBSXG_PROD_RESULT_OP.IxO_FGA_RST;

                    for (int i = start_col; i <= end_col; i++)
                    {
                        int _length = _main_form_vj.fgrid_result[sct_row, i].ToString().Trim().Length;

                        if (_length > 1)
                        {
                            string arg_pcard_id = _main_form_vj.fgrid_result[sct_row, (int)ClassLib.TBSXG_PROD_RESULT_OP.IxPCARD_ID].ToString();
                            string arg_op_cd = _main_form_vj.fgrid_result[fgrid_result.Rows.Fixed - 1, i + 1].ToString();
                            string arg_cmp_cd = get_cmp_cd(arg_op_cd).Rows[0].ItemArray[1].ToString();

                            Click_Save(arg_pcard_id, arg_cmp_cd, arg_op_cd);

                            _main_form_vj.fgrid_result.GetCellRange(sct_row, i).StyleNew.BackColor = Color.Aqua;
                        }
                    }
                }
                else if (tmp_op_cd.Equals("UPS"))
                {
                    ...
                    _main_form_vj.fgrid_result.GetCellRange(sct_row, (int)ClassLib.TBSXG_PROD_RESULT_OP.IxUPS_DIR).StyleNew.BackColor = Color.Aqua;
                }
