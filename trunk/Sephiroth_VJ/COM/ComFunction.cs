...
namespace COM
{
	...
	public class ComFunction
	{
		...
		public static bool Check_Essential_Col(COM.SSP arg_grid)
		{
			int count = 0;
			...
			if(now_row < 0) return true;

			for(int i = 0; i < arg_grid.ActiveSheet.ColumnCount; i++)
			{
				if(arg_grid.arr_essential[i] != "TRUE") continue;
				 
				if(arg_grid.ActiveSheet.Cells[now_row, i].Value == null
					|| arg_grid.ActiveSheet.Cells[now_row, i].Value.ToString().Trim() == "" )
				{
