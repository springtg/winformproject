...
Begin VB.Form frmMO13VJ 
   ...
   Begin VSFlex7LCtl.VSFlexGrid fspLine 
      ...
      FormatString    =   $"frmMO13VJ.frx":0000
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
   ...
   Begin VSFlex7LCtl.VSFlexGrid fspSIZE 
      ...
      ColWidthMax     =   0
      ExtendLastCol   =   0   'False
      FormatString    =   $"frmMO13VJ.frx":0DAA
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
...
Private Sub sbLineDsp(inx As Integer)
   ...
   Else
      SQL = ""
      SQL = SQL & " SELECT LINENO, SUM(TQTY) "
...
Private Sub sbHeadSizeDSP()
   ...
   fspSIZE.Cell(flexcpText, 0, 1, 0, 3) = "Production Results"
   fspSIZE.Cell(flexcpText, 1, 1, 2, 1) = "PO"
   fspSIZE.Cell(flexcpText, 1, 2, 2, 2) = "Style"
   fspSIZE.Cell(flexcpText, 1, 3, 2, 3) = "Model Name"
   fspSIZE.Cell(flexcpText, 0, 4, 0) = "ME"
   fspSIZE.Cell(flexcpText, 1, 4, 1) = "WO"
   fspSIZE.Cell(flexcpText, 2, 4, 2) = "GS"
...
Private Sub sbSizeDSP(arg_arr As Variant)
   ...
   SQL = ""
   SQL = SQL & " SELECT 0, " '0
   SQL = SQL & "        SUBSTR(PO_NO,5,2)||SUBSTR(PO_NO,7,2)||PO_TYPE, " '1
   SQL = SQL & "        SUBSTR(STYLE_CD,1,6)||'-'||SUBSTR(STYLE_CD,7,3), " '2
   SQL = SQL & "        MAX(FN_MM_STYNAME(STYLE_CD)), " '3
   SQL = SQL & "        MAX(FN_GEN(STYLE_CD)), " '4
   SQL = SQL & "        CS_SIZE, " '5
   SQL = SQL & "        SUM(PRS_QTY) " '6
   SQL = SQL & "   FROM MP_PROD "
   ...
   End If
   SQL = SQL & "    AND SEMI_GOOD_CD = '" & varVAL(2) & "' "
   SQL = SQL & "  GROUP BY PO_NO, PO_TYPE, STYLE_CD, CS_SIZE "
   SQL = SQL & "  ORDER BY 2, 3, 6 "
...
On Error GoTo ErrGo
   ...
   For i = 0 To UBound(arrDATA, 2) Step 1
      'row 설정
      If VCOMP <> arrDATA(1, i) & arrDATA(2, i) Then     '기준 변경시
