    ...
    public partial class  Form_EIS_Weekly_Price_Style : COM.APSWinForm.Form_Top
    {
        ...
        private void Init_Form()
        {
            ...
            try
            {
                ...
                ////Title
                //this.Text = "주간 자재 분석 - 스타일";
                //lbl_MainTitle.Text = "주간 자재 분석 - 스타일";
        ...
        private void Event_Tbtn_Print()
        {
            ...
            if (saveFileDialog1.FileName != "")
            {
                ...
                ClassLib.ComFunction.User_Message("Complete Save to Excel file.", "주간 스타일별 원가 분석", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void Display_Warning_Profit_Ratio()
        {
            ...
            for (int i = fgrid_Main.Rows.Fixed; i < fgrid_Main.Rows.Count; i++)
            {
                ...
                if (fgrid_Main.Rows[i].Node.Level == 2) // out_type subtotal
                {
                    ...
                    {
                        break;  // order by 순서가 etc 이하는 모두 etc 데이터이므로 warning 종료해도 무방
                    }
                }
