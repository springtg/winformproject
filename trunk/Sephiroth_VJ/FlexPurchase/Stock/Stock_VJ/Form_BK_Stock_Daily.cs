...
namespace FlexPurchase.Stock.Stock_VJ
{
	public class Form_BK_Stock_Daily : COM.PCHWinForm.Form_Top
	{
		...
		private System.Windows.Forms.MenuItem menuItem_IO_C;
		...
		private void InitializeComponent()
		{
            ...
            this.menuItem_IO_C = new System.Windows.Forms.MenuItem();
            ...
            this.cmb_wareHouse.CaptionStyle = style33;
            ...
            this.cmb_wareHouse.EvenRowStyle = style34;
            ...
            this.cmb_wareHouse.FooterStyle = style35;
            this.cmb_wareHouse.GapHeight = 2;
            this.cmb_wareHouse.HeadingStyle = style36;
            this.cmb_wareHouse.HighLightRowStyle = style37;
            ...
            this.cmb_wareHouse.OddRowStyle = style38;
            ...
            this.cmb_wareHouse.SelectedStyle = style39;
            this.cmb_wareHouse.Size = new System.Drawing.Size(210, 20);
            this.cmb_wareHouse.Style = style40;
            ...
            // 
            this.menuItem_ItemRelation.Index = 4;
            ...
            // 
            this.menuItem_SpecRelation.Index = 5;
            ...
            // 
            this.menuItem_ColorRelation.Index = 6;
            ...
            // menuItem_IO_C
            // 
            this.menuItem_IO_C.Index = 8;
            this.menuItem_IO_C.Text = "Change In/Out Info.";
            this.menuItem_IO_C.Click += new System.EventHandler(this.menuItem_IO_C_Click);
            // 
            // spd_main_Sheet1
            // 
		...
		private string _Source_ItemCd  = "";
		private string _Source_SpecCd  = "";
		private string _Source_ColorCd = "";

		private int _colSTOCK_YMD	     = (int)ClassLib.TBSBK_STOCK_DAILY_VJ.IxSTOCK_YMD;
		private int _colITEM_CD		     = (int)ClassLib.TBSBK_STOCK_DAILY_VJ.IxITEM_CD;
		private int _colITEM_NM		     = (int)ClassLib.TBSBK_STOCK_DAILY_VJ.IxITEM_NAME;
		private int _colSPEC_CD		     = (int)ClassLib.TBSBK_STOCK_DAILY_VJ.IxSPEC_CD;
		private int _colSPEC_NM		     = (int)ClassLib.TBSBK_STOCK_DAILY_VJ.IxSPEC_NAME;
		private int _colCOLOR_CD	     = (int)ClassLib.TBSBK_STOCK_DAILY_VJ.IxCOLOR_CD;
		private int _colCOLOR_NM	     = (int)ClassLib.TBSBK_STOCK_DAILY_VJ.IxCOLOR_NAME;
		private int _colIN_QTY           = (int)ClassLib.TBSBK_STOCK_DAILY_VJ.IxIN_QTY;
		private int _colOUT_QTY          = (int)ClassLib.TBSBK_STOCK_DAILY_VJ.IxOUT_QTY;
...
//			txt_itemName.Text = "";
			...
			string arg_item_name)
		{
			...
			//01.PROCEDURE명
			MyOraDB.Process_Name = "PKG_SBK_STOCK_DAILY_VJ.SELECT_SBK_STOCK";
...
//			} 
		...
		private void menuItem_IO_C_Click(object sender, System.EventArgs e)
		{
			Show_Item_Popup(sender);
		}

		private void Show_Item_Popup(object arg_sender)
		{
			try
			{ 
				...
				MenuItem src = arg_sender as MenuItem;
				
				int vRow = spd_main.ActiveSheet.ActiveRowIndex; 
				string sDiv        = "";
				string sFactory    = cmb_factory.SelectedValue.ToString();
				string sDate	   = "";
				string sWarehouse  = cmb_wareHouse.SelectedValue.ToString();  
				string sItem_cd    = spd_main.ActiveSheet.Cells[vRow, _colITEM_CD].Value.ToString();
				string sSpec_cd    = spd_main.ActiveSheet.Cells[vRow, _colSPEC_CD].Value.ToString();
				string sColor_cd   = spd_main.ActiveSheet.Cells[vRow, _colCOLOR_CD].Value.ToString(); 
				string sItem_name  = spd_main.ActiveSheet.Cells[vRow, _colITEM_NM].Value.ToString();
				string sSpec_name  = spd_main.ActiveSheet.Cells[vRow, _colSPEC_NM].Value.ToString();
				string sColor_name = spd_main.ActiveSheet.Cells[vRow, _colCOLOR_NM].Value.ToString(); 
 

				FlexBase.MaterialBase.Pop_Item_List pop_form = new FlexBase.MaterialBase.Pop_Item_List(true, sItem_cd, sItem_name, sSpec_cd, sSpec_name, sColor_cd, sColor_name);
				pop_form.ShowDialog();

								
				if ((COM.ComVar.Parameter_PopUp2[0].Trim() != sItem_cd )||
					(COM.ComVar.Parameter_PopUp2[2].Trim() != sSpec_cd )||
					(COM.ComVar.Parameter_PopUp2[4].Trim() != sColor_cd) )
				{
					string[] pop_parameter = new string[] { sDiv, sFactory, sDate, sWarehouse, 
															sItem_cd, sSpec_cd, sColor_cd,
															COM.ComVar.Parameter_PopUp2[0].Trim(),
															COM.ComVar.Parameter_PopUp2[2].Trim(),
															COM.ComVar.Parameter_PopUp2[4].Trim()  };

					SAVE_IN_OUT(pop_parameter);
					Tbtn_SearchProcess();
				}
		...
		public bool SAVE_IN_OUT(string[] arg_parameter)
		{
			int col_ct = 12;
			int row_fixed = spd_main.Sheets[0].RowHeader.Rows.Count ; // 그리드 고정행 값
			...
			try
			{
				...
				MyOraDB.Process_Name = "PKG_SBK_STOCK_DAILY_VJ.SAVE_IN_OUT";
				...
				MyOraDB.Parameter_Name[ 7] = "ARG_ITEM_CD_NEW";
				MyOraDB.Parameter_Name[ 8] = "ARG_SPEC_CD_NEW";
				MyOraDB.Parameter_Name[ 9] = "ARG_COLOR_CD_NEW";
				MyOraDB.Parameter_Name[10] = "ARG_REC_DIV";
				...
				// 저장 행 수 구하기
				for (i = 0 ; i < vSelectionRange.Length; i++)
				{
					int start_row = vSelectionRange[i].Row;
					int end_row   = vSelectionRange[i].Row + vSelectionRange[i].RowCount;
				...
				for (i = 0 ; i < vSelectionRange.Length; i++)
				{
					...
					for (int j = start_row ; j < end_row; j++)
					{
						if ( (Convert.ToDouble(spd_main.Sheets[0].Cells[j, _colIN_QTY].Value.ToString())  > 0) &&
						     (Convert.ToDouble(spd_main.Sheets[0].Cells[j, _colOUT_QTY].Value.ToString()) > 0)  )
							MyOraDB.Parameter_Values[para_ct+ 0] = "A";

						else if (Convert.ToDouble(spd_main.Sheets[0].Cells[j, _colIN_QTY].Value.ToString()) > 0)
							MyOraDB.Parameter_Values[para_ct+ 0] = "I";

						else if (Convert.ToDouble(spd_main.Sheets[0].Cells[j, _colOUT_QTY].Value.ToString()) > 0)
							MyOraDB.Parameter_Values[para_ct+ 0] = "O";

						else MyOraDB.Parameter_Values[para_ct+ 0] = "C"; 

						if ((arg_parameter[4] != spd_main.Sheets[0].Cells[j, _colITEM_CD].Value.ToString() ) ||
						    (arg_parameter[5] != spd_main.Sheets[0].Cells[j, _colSPEC_CD].Value.ToString() ) ||
						    (arg_parameter[6] != spd_main.Sheets[0].Cells[j, _colCOLOR_CD].Value.ToString())  )
							MyOraDB.Parameter_Values[para_ct+ 0] = "C";

						
						MyOraDB.Parameter_Values[para_ct+ 1] = arg_parameter[1];
						MyOraDB.Parameter_Values[para_ct+ 2] = spd_main.Sheets[0].Cells[j, _colSTOCK_YMD].Value.ToString();
						MyOraDB.Parameter_Values[para_ct+ 3] = arg_parameter[3];
						MyOraDB.Parameter_Values[para_ct+ 4] = arg_parameter[4];
						MyOraDB.Parameter_Values[para_ct+ 5] = arg_parameter[5];
						MyOraDB.Parameter_Values[para_ct+ 6] = arg_parameter[6];
						MyOraDB.Parameter_Values[para_ct+ 7] = arg_parameter[7];
						MyOraDB.Parameter_Values[para_ct+ 8] = arg_parameter[8];
						MyOraDB.Parameter_Values[para_ct+ 9] = arg_parameter[9];

						if (para_ct == 0)
                            MyOraDB.Parameter_Values[para_ct+10] = "FIRST";
						else
							MyOraDB.Parameter_Values[para_ct+10] = "_____";

						MyOraDB.Parameter_Values[para_ct+11] = ClassLib.ComVar.This_User;
				...
				DataSet ds_Set = MyOraDB.Exe_Modify_Procedure();			// Modify Procedure 실행
