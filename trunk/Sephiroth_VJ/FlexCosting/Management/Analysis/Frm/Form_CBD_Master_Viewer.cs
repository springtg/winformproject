...
{
    public partial class Form_CBD_Master_Viewer : COM.PCHWinForm.Form_Top
    {
        #region Constructor

        public Form_CBD_Master_Viewer()
        {
            InitializeComponent();
        ...
        private void tab_detail_MouseUp(object sender, MouseEventArgs e)
        {
            try
            {
                ...
                else if (e.Button == MouseButtons.Left)
                {
                    ...
                    if (tab_detail.SelectedTab.Name.Equals("tabPage9"))
                    {
                        DisplayMEF();
                    }
                }
        ...
        private void Init_Grid()
        {
            ...
            fgrid_pm_meof_head.Width = Convert.ToInt32(tab_detail.Width * 0.15);
            fgrid_pm_meof_size.Width = Convert.ToInt32(tab_detail.Width * 0.15);
        ...
        private void DisplayMEF()
        {
            try
            {
                if (fgrid_head.Rows.Count > fgrid_head.Rows.Fixed && fgrid_head.Row >= fgrid_head.Rows.Fixed)
                {
                    fgrid_pm_meof_head.Size = new Size(Convert.ToInt32(tab_detail.Width * 0.15), fgrid_pm_meof_head.Height);
                    fgrid_pm_meof_size.Size = new Size(Convert.ToInt32(tab_detail.Width * 0.15), fgrid_pm_meof_size.Height);
