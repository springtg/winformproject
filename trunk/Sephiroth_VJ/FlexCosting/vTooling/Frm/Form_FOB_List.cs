...
namespace FlexCosting.vTooling.Frm
{
    ...
    public partial class Form_FOB_List : COM.PCHWinForm.Form_Top
    {
        ...
        private int _DATACOL_UPD_USER = (int)ClassLib.TBSFX_CBD_TOOLING_AMOTIZATION.IxUPD_USER;
        private int _DATACOL_OBS_ID   = (int)ClassLib.TBSFX_CBD_TOOLING_AMOTIZATION.IxUPD_USER + 1;
        private int _DATACOL_OBS_TYPE = (int)ClassLib.TBSFX_CBD_TOOLING_AMOTIZATION.IxUPD_USER + 2;
        private int _DATACOL_OGAC_YMD = (int)ClassLib.TBSFX_CBD_TOOLING_AMOTIZATION.IxUPD_USER + 3;
        private int _DATACOL_DPO      = (int)ClassLib.TBSFX_CBD_TOOLING_AMOTIZATION.IxUPD_USER + 4;
        private int _DATACOL_FOB      = (int)ClassLib.TBSFX_CBD_TOOLING_AMOTIZATION.IxUPD_USER + 5;
        private int _DATACOL_TOOLING  = (int)ClassLib.TBSFX_CBD_TOOLING_AMOTIZATION.IxUPD_USER + 6;
        private int _KEY_START_COL    = (int)ClassLib.TBSFX_CBD_TOOLING_AMOTIZATION.IxUPD_USER + 7;

        //private string[] copy_dpo = new string[6];
        private CBD copy_cbd = new CBD();
        ...
        private void Init_Control()
        {
            ...
            cmb_Factory.SelectedValue = COM.ComVar.This_Factory.Equals("DS") ? " " : COM.ComVar.This_Factory;
            vDT.Dispose();
            ...
            COM.ComCtl.Set_ComboList(vDT, cmb_category, 1, 2, true, COM.ComVar.ComboList_Visible.Name);
        ...
        private void Display_Title()
        {
            ...
            arg_value[5] = txt_style_cd.Text.Trim().Replace("-", "");
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                // cdcpower_level 이 C로 시작하는 사용자
                if (COM.ComVar.This_CDCPower_Level.StartsWith("C"))
                {
                    fgrid_tooling.Cols[fgrid_tooling.Cols.Count - 1].Visible = true;
                    fgrid_tooling.Cols[_DATACOL_UPD_USER].Visible = true;
                }
                else
                {
                    fgrid_tooling.Cols[fgrid_tooling.Cols.Count - 1].Visible = false;
                    fgrid_tooling.Cols[_DATACOL_UPD_USER].Visible = false;
                }
            }
        ...
        private void DisplayData(DataRow vDR, int iRow, int iStrCol)
        {
            ...
            else if (vCBD.sREV_REASON.Equals("60"))
            {
                fgrid_tooling.GetCellRange(iRow, iStrCol).StyleNew.Font = new Font(fgrid_tooling.Font.FontFamily, (float)8.5, FontStyle.Bold);
                fgrid_tooling.GetCellRange(iRow, iStrCol + 1).StyleNew.Font = new Font(fgrid_tooling.Font.FontFamily, (float)8.5, FontStyle.Bold);
                fgrid_tooling.GetCellRange(iRow, iStrCol + 2).StyleNew.Font = new Font(fgrid_tooling.Font.FontFamily, (float)8.5, FontStyle.Bold);
        ...
        private CBD MakeCBDStruct(DataRow vDR)
        {
            CBD vCBD = new CBD();
        ...
        private void ctxt_confirm_Click(object sender, EventArgs e)
        {
            try
            {
                if (ClassLib.ComFunction.User_Message("Do wan't to Comfirm", "confirm", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)

                if (ConfirmReady())
                {
                    ClassLib.ComFunction.User_Message("Comfirm complete", "confirm", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
                    ClassLib.ComFunction.User_Message("Comfirm faile", "confirm", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            catch (Exception ex)
            {
        ...
        private bool ConfirmReady()
        {
            ...
            object oData = fgrid_tooling.GetCellRange(iRow, iCol).UserData;
            if (oData != null)
            {
                CBD vCBD = (CBD)oData;
                string sDiv = "C";
        ...
        public bool SAVE_SFX_CBD_MASTER_CONFIRM(string arg_div, string arg_dev_fac, string arg_moid, string arg_cbd_id, string arg_cbd_seq, string arg_fob_type_cd, string arg_upd_user)
        {
            try
            {
                ...
                //01.PROCEDURE명
                MyOraDB.Process_Name = "PKG_SFX_CBD_MASTER_SAVE.SAVE_FOB_LIST_CONFIRM";
        ...
        public DataTable SELECT_DPO_ID(string arg_factory, string arg_obs_type)
        {
            try
            {
                ...
                MyOraDB.Parameter_Name[2] = "ARG_THIS_USER";
        ...
        {   
            ...
            CBD vCOPY_CBD = (CBD)copy_cbd;
            CBD vPASTE_CBD = (CBD)fgrid_tooling.GetCellRange(sct_row, sct_col).UserData;
        ...
        private DataTable SAVE_SFX_CBD_DPO(CBD vCOPY_CBD, CBD vPASTE_CBD)
        {
            try
            {
        ...
        private DataTable SAVE_SFX_CBD_COPY_OTHER_STYLE(CBD vCOPY_CBD, CBD vPASTE_CBD, string sDevFac, string sProdFac, string sMOID, string sBOMID)
        {
            try
            {
                ...
                MyOraDB.Parameter_Values[14] = vCOPY_CBD.sREV_REASON;
                MyOraDB.Parameter_Values[15] = "Copy fob list program";
                MyOraDB.Parameter_Values[16] = "C";
                    ...
                    if (obj != null)
                    {
                        ...
                        if (vCBD.sREV_REASON.Equals("x") || vCBD.sREV_REASON.Equals("60"))
                        {
                            if (vCBD.sREV_REASON.Equals("60"))
                            {
                                ...
                                vAnalysis.FormClosed += new FormClosedEventHandler(vCBDFrm_FormClosed);
                            }
                            //else
                            //{
                        ...
                        else
                        {
                            ...
                            {
                                vCBDFrm.FormClosed += new FormClosedEventHandler(vCBDFrm_FormClosed);
        ...
        void vCBDFrm_FormClosed(object sender, FormClosedEventArgs e)
        {
            try
            {
                int row = fgrid_tooling.Row, col = fgrid_tooling.Col;
            ...
            {
                ClassLib.ComFunction.User_Message(ex.Message, "CBD Form closeing", MessageBoxButtons.OK, MessageBoxIcon.Error); ;
            }
        }
        private void mnu_delete_cbd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (oCUR_CBD != null)
                {
                    CBD vCUR_CBD = (CBD)oCUR_CBD;

                    if (!vCUR_CBD.sREV_REASON.Equals("x"))
                    {
                        if (SAVE_SFX_CBD_DPO(vCUR_CBD))
                        {
                            DataTable vDT = SELECT_SFX_CBD_TOOL(vCUR_CBD.sDEV_FAC, vCUR_CBD.sMOID, vCUR_CBD.sCBD_ID, vCUR_CBD.sFOB_TYPE_CD, vCUR_CBD.sCBD_SEQ);
                            if (vDT != null && vDT.Rows.Count == 1)
                            {
                                MessageBox.Show("Delete complete.");
                                DisplayData(vDT.Rows[0], vCUR_CBD.iRow, vCUR_CBD.iStrCol);
                            }
                            /*
        ...
        private bool SAVE_SFX_CBD_DPO(CBD vCUR_CBD)
        {
            try
            {
